  ...
  describe(`with parser: ${parser}`, function () {
    ...
    describe(`findImports`, function () {
      ...
      describe(`for import statement`, function () {
        ...
        it(`works for string literals in destructured require defaults`, function () {
          testCase(
            `const foo = require('foo')`,
            `const {'string-literal': test} = require('foo')`,
            { test: `foo['string-literal']` }
          )
        })
        it(`works for string literals in destructured require defaults 2`, function () {
          testCase(
            `const {'string-literal': foo} = require('foo')`,
            `const {'string-literal': test} = require('foo')`,
            { test: `foo` }
          )
        })
